...
impl Lehmer {
    ...
    /// Approximate size of the code can be computed with `Lehmer::get_encode_size`
    /// The code can later be decoded with `Lehmer::decode`
    ///
    /// # Errors
    ///
    ...
    /// Decodes a lehmer code generated by `Lehmer::encode`  
    ...
    pub fn decode(encoded: &[u8], results: &mut [u32]) -> Result<(), Error> {
        ...
        // Stores big number mod / div results
        let mut intermediate = vec![0; results.len()];

        let mut input: UBig = UBig::from_le_bytes(encoded);

        for (index, t) in intermediate[0..results.len() - 1].iter_mut().enumerate() {
        ...
        for (index, &t) in intermediate[0..results.len() - 1].iter().rev().enumerate() {
